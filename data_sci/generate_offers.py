...
def generate_random_offer(list_price):
    ...
    escrow_fluctuation = random.uniform(-0.8, 2)
    offer_expiry_fluctuation = random.uniform(-0.8, 0.8)
    # financing (vars)
    offer['offer_price'] = ceil((1 + price_fluctuation) * list_price) # round off to nearest int
    offer['all_cash'] = random.choice(['Y','N']) # boolean
    proportion_down_payment = random.random() if offer['all_cash'] == 'N' else 0.0 # percentage
    offer['proportion_down_payment'] = round(proportion_down_payment, 2)
    

    # financing terms (do we need more variables)
    offer['financing_contingency'] = random.choice(['Y','N']) # boolean
    offer['appraisal_contingency'] = random.choice(['Y','N'])
    offer['preapproved_loan'] = random.choice(['Y','N']) if offer['all_cash'] == 'N' else 'N' # boolean
    ...
    # other terms
    offer['offer_expiry'] = ceil(3 * (1 + offer_expiry_fluctuation)) # days
...
def write_offers(offers):
    with open('offers.csv', 'w') as csvfile:
        field_names = ['list_price', 'offer_price', 'all_cash', 'proportion_down_payment', 'financing_contingency', 'appraisal_contingency', 
                ...
                'property_as_is', 'property_inspection_contingency', 'pest_control', 'repairs_payment', 'num_disclosures_signed', 'offer_expiry',
